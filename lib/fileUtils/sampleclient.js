...
const { state, config } = require('..');

const { FILES_PATH } = config

const keyPath = path.join(FILES_PATH, 'oauth2.keys.json');
let keys = {
...
class SampleClient {
  constructor(options) {
    ...
    this.oAuth2Client.on('tokens', tokens => {
      state.oAuthToken = { ...state.oAuthToken, ...tokens }
    })
  }

  async authenticate(scopes, state){
    let token;
    if(state.oAuthToken && state.oAuthToken){
      token = state.oAuthToken
    } else {
      token = await this.getToken(scopes)
    }
