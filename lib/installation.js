module.exports = async ({ config, fs, exec, createLink, console }) => {
    const { FILES_PATH } = config
    if (!fs.existsSync(FILES_PATH)) {
        let useExample = await console.prompt('Initializing a `My-Cohorts` folder. Would you like to download example lecture templates?', ['Yes', 'No'])
